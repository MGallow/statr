...
# Bridge regression estimators

#calculates the beta estimates for bridge regression
bridge = function(X, y, lam, alpha, tol = 10^(-5), maxit = 1000){
  ...
  returns = list(b1 = b1, b = b, total.iterations = iteration, grads = grads)
  return(returns)
}
...
#' @title Bridge Gradient
#' @description Computes the gradient of bridge regression. Use to determine if KKT conditions are satisfied
#'
#' @param XX cross-product(X, X)
#' @param Xy cross-product(X, y)
...
bridge_gradient = function(XX, Xy, b, lam, alpha){
  ...
  #gradient for beta
  grad_b = -Xy + XX %*% b + lam*(abs(b)^(alpha - 1))*sign(b)
