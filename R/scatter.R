...
multiplot <- function(..., plotlist = NULL, cols = 1, layout = NULL) {
    ...
    if (is.null(layout)) {
        # Make the panel ncol: Number of columns of plots nrow: Number of rows needed, calculated
        # from # of cols
...
scatter = function(data., x., y.) {
    ...
    # create scatterplot
    ggplot(data.) + geom_point(mapping = aes(x = eval(x., data.), y = eval(y., data.))) + 
        ggtitle("Scatterplot") + ylab(y.) + xlab(x.)
...
diagnostic = function(data., x., y.) {
    ...
    residual_plot = ggplot(data., mapping = aes(x = fit$fitted.values, y = fit$residuals)) + 
        geom_abline(intercept = 0, slope = 0, color = "red") + geom_point() + ggtitle("Residual Plot") + 
    ...
    qqplot = ggplot(data.) + stat_qq(mapping = aes(sample = eval(y., data.))) + geom_abline(intercept = int, 
        slope = slope, color = "red") + ggtitle("QQ Plot")
