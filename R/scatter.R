...
scatter = function(data., x., y.) {
    ...
    # create scatterplot
    ggplot(data.) + geom_point(mapping = aes(x = eval(x., data.), y = eval(y., data.))) + ggtitle("Scatterplot") + 
        ylab(y.) + xlab(x.)
...
diagnostic = function(data., x., y.) {
    ...
    residual_plot = ggplot(data., mapping = aes(x = fit$fitted.values, y = fit$residuals)) + geom_abline(intercept = 0, 
        slope = 0, color = "red") + geom_point() + ggtitle("Residual Plot") + ylab("residuals") + 
        xlab("fitted values")
