...
How fast is this function? For illustration purposes, we will compare the underlying c++ function `linearc` to the `lm.fit` built-in function in R.

```{r message = FALSE}

#benchmark both functions
microbenchmark::microbenchmark(linearc(as.matrix(X), as.matrix(y), intercept = F), lm.fit(as.matrix(X), as.matrix(y)))


```

We can see that on average, our function edges out R's built-in function (though to be fair, `lm.fit` is performing a few other calculations).

Next, we will try **ridge regression**. This function defaults to a sequence of `lam` values and will use cross validation to choose the optimal tuning parameter -- therefore, we only need specify the penalty as "ridge":
...
We will use a linear model fit to generate new predictions. This time, let us specify the `lam` tuning parameter to `lam = 0.1`.
...
Again, this function defaults to a sequence of `lam` values and will use cross validation to choose the optimal tuning parameter -- therefore, we only need specify the penalty as "ridge":
...
Note this time we specify the `lam` tuning parameter as 0.1 (optional).
...
#timeit bridge penalized logistic regression
