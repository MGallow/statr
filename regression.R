...
RIDGE = function(X, Y, lam = NULL, intercept = FALSE, standardize = FALSE){
  ...
  # is lam specified?
  if (is.null(lam)){
    lam =  glmnet::cv.glmnet(x = X, y = Y, alpha = 0, intercept = intercept, family = family, standardize = standardize)$lambda.min
  }
  
  # calculate coefficients
  betas = glmnet(x = X, y = Y, standardize = standardize, intercept = intercept, family = family, alpha = 0, lambda = lam)
...
LASSO = function(X, Y, lam = NULL, intercept = FALSE, standardize = FALSE){
  ...
  # is lam specified?
  if (is.null(lam)){
    lam =  glmnet::cv.glmnet(x = X, y = Y, alpha = 1, intercept = intercept, family = family, standardize = standardize)$lambda.min
  }
  
  # calculate coefficients
  betas = glmnet(x = X, y = Y, standardize = standardize, intercept = intercept, family = family, alpha = 1, lambda = lam)
  betas = as.matrix(do.call(cbind, coef(betas))[-1,])
...
CVsplit = function(X, Y, split = 0.5){
  ...
  # specify leave.out
  leave.out = samply(nrow(X), floor(nrow(X)*split))
  ...
  returns = list(X.train = X.train, Y.train = Y.train, X.test = X.test, Y.test = Y.test)
  return(returns)
